...
func upgrade() {
	...
	podmanCmd := fmt.Sprintf(`sudo podman run `+
		...
		`ansible-playbook -i %s@%s, --private-key /runner/env/ssh_key -e "quay_image=%s redis_image=%s postgres_image=%s quay_hostname=%s local_install=%s quay_root=%s" upgrade_mirror_appliance.yml %s %s`,
		sshKey, targetUsername, targetHostname, quayImage, redisImage, postgresImage, quayHostname, strconv.FormatBool(isLocalInstall()), quayRoot, askBecomePassFlag, additionalArgs)
